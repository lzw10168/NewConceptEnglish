...
def main():
    ...
    # Starting with lesson 234 -> 001
    if '234' not in mapping:
        mapping['234'] = 1  # Lesson ID 234 corresponds to Lesson 001

    # Process lessons 234 to 305
    for lesson_id in range(234, 282):  # 306 is exclusive, so it will process 234-305
        # Calculate lesson_num
        if str(lesson_id) in mapping:
            lesson_num = mapping[str(lesson_id)]
        else:
            # Lesson number is lesson_id minus 233
            lesson_num = lesson_id - 233
