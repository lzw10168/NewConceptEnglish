...
def main():
    ...
    # Starting with lesson 2 -> 001
    if '2' not in mapping:
        mapping['2'] = 1  # Lesson ID 2 corresponds to Lesson 001

    # Process lessons 2 to 73
    for lesson_id in range(2, 74):  # 74 is exclusive, so it will process 2-73
        # Calculate lesson_num
        if str(lesson_id) in mapping:
            lesson_num = mapping[str(lesson_id)]
        else:
            # Lesson number is lesson_id minus 1
            lesson_num = lesson_id - 1
